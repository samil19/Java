...
public class CorteDataAccess {
    ...
    public void corte(int CorteID, String Nickname){
        try {
            int egresos=0;
            int ingresos=0;
            int corte;
            TransaccionDataAccess da = new TransaccionDataAccess();
             String sqle = "select * from Transaccion where TipoTransaccion = 'Egreso' AND Nickname = '" +Nickname+"'";
            ResultSet rs = DBUtils.getPreparedStatement(sqle).executeQuery();
            while(rs.next()){
                Transaccion r = new Transaccion(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5), rs.getString(6));
                egresos -= rs.getInt(5);
                da.delete(rs.getInt(1));
            }
            
            
            String sqli = "select * from Transaccion where TipoTransaccion = 'Ingreso' AND Nickname = '" +Nickname+"'";
            ResultSet ri = DBUtils.getPreparedStatement(sqli).executeQuery();
            while(ri.next()){
                Transaccion r = new Transaccion(ri.getInt(1), ri.getString(2), ri.getString(3), ri.getString(4), ri.getInt(5), ri.getString(6));
    ...
    public void corte2(int eg, int in, int corte, String nickname){
        ...
        try {
            ...
            while(rt.next()){
            Corte n= new Corte(rt.getInt(1), rt.getInt(2), rt.getInt(3), rt.getInt(4), rt.getString(5));
            ID+=rt.getInt(1);
            }
